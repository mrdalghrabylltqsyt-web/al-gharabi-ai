...
add('instagram-onboarding-response-type-token', /platform === 'instagram' && instagramOnboarding/.test(read('engine/social/oauth.ts')) && /params\.response_type = 'token'/.test(read('engine/social/oauth.ts')), 'Instagram يستخدم response_type=token (لا code) وفق التدفّق الرسمي');
add('facebook-oauth-untouched-by-instagram', /response_type: 'code',[\s\S]{0,80}\n  \};/.test(read('engine/social/oauth.ts')) && /platform === 'instagram' && instagramOnboarding/.test(read('engine/social/oauth.ts')), 'التدفّق الرسمي مقيّد بـinstagram؛ Facebook يبقى response_type=code');
