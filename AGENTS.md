...
- **معاينة الجوال (اختياري)**: لتجربة الواجهة من متصفح الهاتف بدون Resend/Google، اضبط `GHARABI_PREVIEW_TOKEN` في بيئة الخادم (قيمة عشوائية قوية، لا تُكتب في Git). عندها يقبل `/api/auth/preview-login` صيغتين — `GET ?token=...` لرابط الإشارة المرجعية، و`POST { token }` للواجهة (التوكن في الجسم فلا يظهر في سجلات الخادم ولا في محفوظات المتصفح). فتح `/#preview_token=...` (المفضّل) أو `/?preview_token=...` (توافق قديم) يسجّل الدخول تلقائياً ثم يمسح التوكن من الرابط فوراً، فيمنح رابطاً محفوظاً واحداً وصولاً دائماً بلا OTP. بدون ضبط المتغير يعيد المسار **404** كأنه غير موجود، والمقارنة بزمن ثابت (`timingSafeEqual`)، وحدّ 10 محاولات لكل IP كل 15 دقيقة. لا يُسجَّل التوكن ولا يُعاد في أي استجابة خطأ. اختبار `engine/tests/preview.login.test.ts` يثبت هذه الشروط.
